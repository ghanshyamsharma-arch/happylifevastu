...
<?php
    $googleApiKey = '';
    try {
        $response = Http::withoutVerifying()->post(url('/') . '/api/getSystemFlag')->json();
        
        // Check if response is valid and has recordList
        if (!empty($response) && is_array($response) && isset($response['recordList']) && is_array($response['recordList'])) {
            $getsystemflag = collect($response['recordList']);
            $apikey = $getsystemflag->where('name', 'googleMapApiKey')->first();
            $googleApiKey = $apikey['value'] ?? '';
        } else {
            \Log::warning('System flag API returned invalid response in kundali.blade.php', ['response' => $response]);
        }
    } catch (\Exception $e) {
        \Log::error('Failed to fetch system flag in kundali.blade.php: ' . $e->getMessage());
    }
?>
...
<?php else: ?>
    <script>
        console.warn('Google Maps API key not available in kundali page');
        // Provide fallback - disable Google Places functionality
        var originAutocomplete = { getPlace: function() { return null; } };
    </script>
<?php endif; ?>
...
<script>
    ...
    if (input && typeof google !== 'undefined' && google.maps) {
        ...
        function getTimezone(lat, lng) {
            ...
            var timezoneApiUrl = `https://maps.googleapis.com/maps/api/timezone/json?location=${lat},${lng}&timestamp=${timestamp}&key=<?php echo e($googleApiKey); ?>`;
        ...
        console.warn('Google Maps not loaded or input not found');
        var originAutocomplete = { getPlace: function() { return null; } };
    }
    <?php else: ?>
    ...
    // Rest of your JavaScript functions (updateAmount, etc.) remain the same
    ...
    $(document).ready(function() {
        // Your existing jQuery code remains the same
        function showLoader() {
            $('#overlay').fadeIn(); 
        }
        ...
        $('#showKundalibtn').click(function(e) {
            // Your existing validation and AJAX code remains exactly the same
            ...
            <?php if(empty($googleApiKey)): ?>
            // Skip place validation if no API key
            <?php else: ?>
            var place = originAutocomplete.getPlace();
                        ...
                        if (response.error) {
                            ...
                            for (var field in errorMessages) {
                                if (errorMessages.hasOwnProperty(field)) {
                                    toastr.error(errorMessages[field][0]);
                                }
                            }
                        } else {
    ...
    // Your existing delete kundali code remains exactly the same
    $(document).ready(function() {
    ...
    $(document).ready(function() {
        function toggleLanguageDropdown() {
            ...
            if (selectedType === 'basic' || selectedType === 'Select Type') {
                $('#languageDropdown').addClass('d-none');
            } else {
...
<?php echo $__env->make('frontend.layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/happylifevastu/public_html/resources/views/frontend/pages/kundali.blade.php ENDPATH**/ ?>