...
<div class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?php echo e(route('front.pujariSlots')); ?>" class="active">
            <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
            Slots
        </a>
...
<div style="background:#dcfce7;padding:10px 14px;border-radius:8px;color:#166534;margin-bottom:14px;font-size:13px;">\u2705 <?php echo e(session('success')); ?></div>
<?php endif; ?>
<?php if(session('error')): ?>
<div style="background:#fee2e2;padding:10px 14px;border-radius:8px;color:#991b1b;margin-bottom:14px;font-size:13px;">\u26a0\ufe0f <?php echo e(session('error')); ?></div>
<?php endif; ?>
...
<div style="background:white;border-radius:14px;padding:20px;box-shadow:0 1px 4px rgba(0,0,0,0.07);margin-bottom:20px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;">
        <?php $__currentLoopData = $recurring; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $pct = $slot->maxBookings > 0 ? round(($slot->bookedCount / $slot->maxBookings) * 100) : 0; ?>
        <div id="slotCard_<?php echo e($slot->id); ?>" style="border:1.5px solid #f1f5f9;border-radius:10px;padding:14px;background:#fafafa;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
                <div>
                    <div style="font-weight:700;font-size:15px;"><?php echo e($days[$slot->dayOfWeek] ?? '--'); ?></div>
                    <div style="font-size:13px;color:#64748b;margin-top:2px;">
                        <?php echo e(date('h:i A', strtotime($slot->startTime))); ?> – <?php echo e(date('h:i A', strtotime($slot->endTime))); ?>

                    </div>
                </div>
                <span id="badge_<?php echo e($slot->id); ?>"
                      style="padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;
                             <?php echo e($slot->status === 'active' ? 'background:#f0fdf4;color:#16a34a;' : ($slot->status === 'full' ? 'background:#fffbeb;color:#d97706;' : 'background:#fef2f2;color:#dc2626;')); ?>">
            ...
            <div style="margin-bottom:8px;">
                <div style="display:flex;justify-content:space-between;font-size:12px;color:#64748b;margin-bottom:3px;">
                    <span>Bookings</span><span><?php echo e($slot->bookedCount); ?>/<?php echo e($slot->maxBookings); ?></span>
                </div>
                <div style="background:#e2e8f0;border-radius:4px;height:6px;">
                    <div style="background:<?php echo e($pct >= 100 ? '#ef4444' : ($pct >= 70 ? '#f59e0b' : '#22c55e')); ?>;height:6px;border-radius:4px;width:<?php echo e($pct); ?>%;"></div>
                </div>
            </div>
            <?php if($slot->note): ?>
            <div style="font-size:12px;color:#94a3b8;margin-bottom:8px;">\ud83d\udccc <?php echo e($slot->note); ?></div>
            <?php endif; ?>
            <div style="display:flex;gap:6px;">
                <?php if($slot->status !== 'full'): ?>
                <button onclick="toggleStatus(<?php echo e($slot->id); ?>, this)"
                        ...
                        style="flex:1;padding:6px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid;
                               <?php echo e($slot->status === 'active' ? 'background:#fef2f2;color:#dc2626;border-color:#fca5a5;' : 'background:#f0fdf4;color:#16a34a;border-color:#86efac;'); ?>">
                    <?php echo e($slot->status === 'active' ? 'Deactivate' : 'Activate'); ?>
...
<div style="background:white;border-radius:14px;padding:20px;box-shadow:0 1px 4px rgba(0,0,0,0.07);margin-bottom:20px;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;">
        <?php $__currentLoopData = $specific; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $slot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $pct = $slot->maxBookings > 0 ? round(($slot->bookedCount / $slot->maxBookings) * 100) : 0; ?>
        <div id="slotCard_<?php echo e($slot->id); ?>" style="border:1.5px solid #f1f5f9;border-radius:10px;padding:14px;background:#fafafa;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
                <div>
                    <div style="font-weight:700;font-size:15px;"><?php echo e($slot->slotDate ? \Carbon\Carbon::parse($slot->slotDate)->format('d M Y') : '--'); ?></div>
                    <div style="font-size:13px;color:#64748b;margin-top:2px;">
                        <?php echo e(date('h:i A', strtotime($slot->startTime))); ?> \u2013 <?php echo e(date('h:i A', strtotime($slot->endTime))); ?>
            ...
            <?php if($slot->note): ?><div style="font-size:12px;color:#94a3b8;margin-bottom:8px;">\ud83d\udccc <?php echo e($slot->note); ?></div><?php endif; ?>
            <div style="display:flex;gap:6px;">
                <?php if($slot->status !== 'full'): ?>
                <button onclick="toggleStatus(<?php echo e($slot->id); ?>, this)"
                        data-status="<?php echo e($slot->status); ?>"
        ...
        <div style="padding:24px;">
            ...
            <div id="weekdayWrap" style="margin-bottom:16px;">
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;" id="dayBtnsWrap">
                    ...
                    <button type="button" onclick="pickDay(<?php echo e($di); ?>, this)"
                            class="dayPickBtn" data-day="<?php echo e($di); ?>"
            ...
            <div id="specificWrap" style="display:none;margin-bottom:16px;">
                ...
                <input type="date" id="slotSpecificDate" min="<?php echo e(date('Y-m-d')); ?>"
...
function savePortalSlot() {
   ...
   const url = id
    ? `<?php echo e(url('/pujari-portal/slots')); ?>/${id}`
    : '<?php echo e(route("front.pujariSlotsCreate")); ?>';
...
function toggleStatus(id, btn) {
   fetch('<?php echo e(route("front.pujariSlotsToggleStatus")); ?>', {
    method: 'POST',
    headers: {
...
function deleteSlot(id) {
    ...
    fetch('<?php echo e(route("front.pujariSlotsDelete")); ?>', {
    method: 'DELETE',
    headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
...
</script><?php /**PATH /home/happylifevastu/public_html/resources/views/frontend/pujari/my-slots.blade.php ENDPATH**/ ?>