                            ...
                            <div class="bg-gray looks-1 d-flex p-2 py-3 p-sm-3 overflow-hidden position-relative flex-column-reverse flex-sm-row">
                                ...
                                <div class="looks-image ilook2 text-center position-relative align-items-center mr-md-3 mr-lg-4">
                                    <div class="looks-img-box position-relative">
                                        <img src="<?php echo e(asset('public/frontend/astrowaycdn/astroway/web/content/images/ads/kundli-matching-image.svg')); ?>" alt="Indian Bride Image">
                                    </div>
                                </div>
...
<?php
    ...
    try {
        ...
        if (!empty($response) && is_array($response) && isset($response['recordList']) && is_array($response['recordList'])) {
            ...
            \Log::warning('System flag API returned invalid response', ['response' => $response]);
        }
    } catch (\Exception $e) {
        \Log::error('Failed to fetch system flag: ' . $e->getMessage());
    }
?>
...
<?php else: ?>
    <!-- Google Maps API key not available - using fallback -->
    <script>
        console.warn('Google Maps API key not available. Address autocomplete disabled.');
        function initializeAutocomplete() {
            console.log('Address autocomplete is disabled');
        }
    </script>
<?php endif; ?>
...
<script>
    ...
    // Initialize autocomplete only if Google Maps is loaded
...
<?php echo $__env->make('frontend.layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/happylifevastu/public_html/resources/views/frontend/pages/kundali-matching.blade.php ENDPATH**/ ?>