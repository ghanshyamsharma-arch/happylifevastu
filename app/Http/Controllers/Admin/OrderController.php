...
// Add new method to OrderController:
public function getOrderDetail(Request $request, $orderId)
{
    $order = UserOrder::join('users as u', 'u.id', '=', 'order_request.userId')
        ->leftJoin('order_addresses as a', 'a.id', '=', 'order_request.orderAddressId')
        ->where('order_request.id', $orderId)
        ->select('order_request.*', 'u.name as userName', 'u.contactNo', 'u.email',
            'a.name as addrName', 'a.flatNo', 'a.locality', 'a.landmark',
            'a.city', 'a.state', 'a.pincode', 'a.phoneNumber')
        ->first();
    ...
    return view('pages.admin-order-detail', compact('order', 'orderItems', 'currency'));
}
        public function downloadInvoice($id)
        {
            // Fetch order details based on $id
            $order = UserOrder::where('order_request.id', '=', $id)
                ->where('order_request.orderType', '=', 'astromall')
                ->join('users as us', 'us.id', '=', 'order_request.userId')
                ->leftjoin('order_addresses as address', 'address.id', '=', 'order_request.orderAddressId')
                ->select('order_request.*', 'us.name as userName', 'us.contactNo as userContactNo','us.email as userEmail',
                    'address.name as addressUserName', 'address.phoneNumber', 'address.phoneNumber2', 'address.flatNo', 'address.locality', 'address.landmark', 'address.city', 'address.state', 'address.country', 'address.pincode'
                )
                ->first();
            ...
            // Generate PDF invoice view with order data and items
            $pdf = PDF::loadView('pages.invoice', compact('order', 'orderItems', 'currencySymbol', 'logo', 'gst', 'appname'));
            return $pdf->stream();
        }
