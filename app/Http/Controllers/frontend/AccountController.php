...
class AccountController extends Controller
{
    ...
    public function myPujariSlotBookings()
{
    if (!authcheck()) {
        return redirect()->route('front.home');
    }
    ...
    // Fetch bookings with pujari name
    $bookings = PujariBooking::select(
            'pujari_bookings.*',
            'pujaris.name as pujariName',
            'pujaris.profileImage as profileImage'
        )
        ->leftJoin('pujaris', 'pujaris.id', '=', 'pujari_bookings.pujariId')
        ->where('pujari_bookings.userId', $userId)
        ->orderBy('pujari_bookings.bookingDate', 'desc')
        ->paginate(10);

    return view('frontend.pages.my-pujari-slot-bookings', [
        'bookings' => $bookings,
        'user' => $user
    ]);
}
