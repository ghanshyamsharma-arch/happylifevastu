                ...
                <tbody>
                    ...
                    @foreach ($orderRequest as $request)
                        <tr class="intro-x">
                            ...
                            {{-- ===== UPDATED: Multi-item product column ===== --}}
                            <td>
                                @if(isset($request->orderItems) && $request->orderItems->count() > 0)
                                    {{-- Multi-item order --}}
                                    <div class="d-flex align-items-center" style="gap:6px;">
                                        ...
                                        <div class="w-10 h-10 image-fit zoom-in flex-shrink-0">
                                            <img class="rounded-full cursor-pointer"
                                                 src="{{ Str::startsWith($request->orderItems->first()->productImage ?? '', ['http://','https://']) ? $request->orderItems->first()->productImage : '/' . ($request->orderItems->first()->productImage ?? '') }}"
                                                 onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                                 alt="Product image"
                                                 onclick="openImage('{{ Str::startsWith($request->orderItems->first()->productImage ?? '', ['http://','https://']) ? $request->orderItems->first()->productImage : '/' . ($request->orderItems->first()->productImage ?? '') }}')" />
                                        </div>
                                        <div>
                                            <span class="font-medium">{{ $request->orderItems->first()->productName }}</span>
                                            @if($request->orderItems->count() > 1)
                                                <br>
                                                <a href="javascript:;" 
                                                   onclick="toggleOrderItems({{ $request->id }})"
                                                   class="text-primary small">
                                                    +{{ $request->orderItems->count() - 1 }} more item(s)
                                                </a>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    @if($request->orderItems->count() > 1)
                                    <div id="order-items-{{ $request->id }}" style="display:none;margin-top:8px;">
                                                ...
                                                <tr>
                                                    <td style="padding:4px 8px;">
                                                        <div class="d-flex align-items-center" style="gap:4px;">
                                                            <img src="{{ Str::startsWith($item->productImage ?? '', ['http://','https://']) ? $item->productImage : '/' . ($item->productImage ?? '') }}"
                                                                 onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                                    ...
                                                    <td style="padding:4px 8px;">
                                                        {{ $currency->value }}{{ number_format($item->totalPrice ?? $item->unitPrice ?? 0, 2) }}
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                ...
                                @else
                                    ...
                                    <div class="flex">
                                        ...
                                        <span class="ml-2 self-center">{{ $request->productName ?? '' }}</span>
                                    </div>
                                @endif
                            </td>
                            {{-- ===== END: Product column ===== --}}
                            ...
                            <td class="text-center">
                                <a href="{{ route('admin.order.detail', ['orderId' => $request->id]) }}" 
                            ...
                            <td class="text-center">
                                <a target="_blank" href="{{ route('order.invoice', ['id' => $request->id]) }}" class="btn btn-primary">
