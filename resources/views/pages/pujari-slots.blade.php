...
<div class="intro-y flex flex-wrap items-center gap-3 mt-5">
    <form action="{{ route('pujariSlots') }}" method="POST" class="flex flex-wrap gap-2 items-center">
        @csrf
        ...
        <select name="pujariId" class="form-select w-40 box">
            ...
            <option value="{{ $p->id }}" {{ ($pujariId ?? '') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
            @endforeach
        </select>
        <select name="dayType" class="form-select w-36 box">
            <option value="">All Types</option>
            <option value="specific"  {{ ($dayType ?? '') == 'specific'  ? 'selected' : '' }}>Specific Date</option>
            <option value="recurring" {{ ($dayType ?? '') == 'recurring' ? 'selected' : '' }}>Recurring</option>
        </select>
        ...
        <a href="{{ route('pujariSlots') }}" class="btn btn-secondary">
...
<div class="intro-y col-span-12 overflow-auto lg:overflow-visible mt-5">
    <table class="table table-report -mt-2">
        ...
        <tbody>
            ...
            <tr class="intro-x" id="slotRow_{{ $slot->id }}">
                <td>{{ ($page - 1) * 20 + ++$no }}</td>
                <td>
                    <div class="flex items-center gap-2">
                       <img src="{{ url('public/' . $slot->pujariImage) }}"
     onerror="this.src='{{ asset('build/assets/images/person.png') }}'"
     class="w-8 h-8 rounded-full object-cover">
                            
                        <div>
                            <div class="font-medium whitespace-nowrap">{{ $slot->pujariName }}</div>
                            <div class="text-slate-500 text-xs">{{ $slot->pujariContact }}</div>
                        </div>
                    </div>
                </td>
                <td class="text-center">
                    @if($slot->dayType === 'recurring')
                        <span class="font-medium">{{ $days[$slot->dayOfWeek] ?? '--' }}</span>
                        <div class="text-slate-400 text-xs">Every week</div>
                    @else
                        <span class="font-medium">{{ $slot->slotDate ? \Carbon\Carbon::parse($slot->slotDate)->format('d M Y') : '--' }}</span>
                    @endif
                </td>
                <td class="text-center font-medium">
                    {{ date('h:i A', strtotime($slot->startTime)) }} -
                    {{ date('h:i A', strtotime($slot->endTime)) }}
                </td>
                <td class="text-center">
                    <div class="font-medium">{{ $slot->bookedCount }}/{{ $slot->maxBookings }}</div>
                    <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                        @php $pct = $slot->maxBookings > 0 ? round(($slot->bookedCount / $slot->maxBookings) * 100) : 0; @endphp
                        <div class="h-1.5 rounded-full {{ $pct >= 100 ? 'bg-red-500' : ($pct >= 70 ? 'bg-yellow-400' : 'bg-green-500') }}"
                             style="width:{{ $pct }}%"></div>
                    </div>
                </td>
                <td class="text-center">
                    <span class="px-2 py-0.5 rounded text-xs font-semibold
                        {{ $slot->dayType === 'recurring' ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700' }}">
                        {{ $slot->dayType === 'recurring' ? 'Recurring' : 'Specific' }}
                    </span>
                </td>
                <td class="text-center">
                    <button onclick="toggleSlotStatus({{ $slot->id }}, this)"
                            class="btn btn-sm {{ $slot->status === 'active' ? 'btn-success' : ($slot->status === 'full' ? 'btn-warning' : 'btn-danger') }} py-1 px-3"
                            data-status="{{ $slot->status }}"
                            {{ $slot->status === 'full' ? 'disabled' : '' }}>
                        {{ ucfirst($slot->status) }}
                    </button>
                </td>
                <td class="text-center text-slate-500 text-xs">{{ $slot->note ?? '--' }}</td>
                <td class="text-center">
    ...
    <nav class="w-full sm:w-auto sm:mr-auto">
        <ul class="pagination">
            ...
            <li class="page-item {{ $page == $i ? 'active' : '' }}">
                <a class="page-link" href="{{ route('pujariSlots') }}?page={{ $i }}&searchString={{ $searchString ?? '' }}&pujariId={{ $pujariId ?? '' }}&dayType={{ $dayType ?? '' }}&status={{ $status ?? '' }}">{{ $i }}</a>
            </li>
            @endfor
        </ul>
    </nav>
    <div class="hidden md:block text-slate-500 ml-auto">
        Showing {{ $start ?? 1 }} to {{ $end ?? $totalRecords }} of {{ $totalRecords }} entries
    </div>
</div>
...
<div class="intro-y col-span-12 mt-10">
    <div class="box p-10 text-center">
        <img src="{{ asset('build/assets/images/nodata.png') }}" class="w-24 mx-auto mb-4" onerror="this.style.display='none'">
        ...
        <div style="padding:24px;">
            ...
            <div id="dayOfWeekWrap" style="margin-bottom:16px;">
                ...
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                    ...
                    <button type="button" onclick="selectSlotDay({{ $di }}, this)"
                            class="daySelectBtn" data-day="{{ $di }}"
            ...
            <div id="specificDateWrap" style="display:none;margin-bottom:16px;">
                ...
                <input type="date" id="slotDate" min="{{ date('Y-m-d') }}"
...
function toggleSlotStatus(id, btn) {
    ...
    fetch('{{ route("togglePujariSlotStatus") }}', {
