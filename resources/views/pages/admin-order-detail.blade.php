...
<div class="intro-y grid grid-cols-12 gap-5 mt-5">
    ...
    <div class="col-span-12 lg:col-span-8">
                ...
                <table class="table table-bordered" style="min-width:460px;">
                                ...
                                @if($item->productImage)
                                    <div class="w-10 h-10 image-fit zoom-in flex-shrink-0 mx-auto">
                                        <img class="rounded-lg cursor-pointer"
                                             src="{{ Str::startsWith($item->productImage ?? '', ['http://','https://']) ? $item->productImage : '/' . ($item->productImage ?? '') }}"
                                             onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                             alt="{{ $item->productName }}"
                                             onclick="viewImage('{{ Str::startsWith($item->productImage ?? '', ['http://','https://']) ? $item->productImage : '/' . ($item->productImage ?? '') }}')">
                                    </div>
                                @else
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td class="text-center text-slate-500">{{ $currSym }}{{ number_format(($order->payableAmount * $order->gstPercent / 100), 2) }}</td>
                        </tr>
                        @endif
        ...
        <div class="intro-y box p-5">
            ...
            @if($order->addrName || $order->flatNo)
            <div class="grid grid-cols-2 gap-3 text-sm">
                <div>
                    <span class="text-slate-500">Recipient Name</span>
                    <div class="font-medium mt-1">{{ $order->addrName ?? '-' }}</div>
                </div>
                <div>
                    <span class="text-slate-500">Phone</span>
                    <div class="font-medium mt-1">{{ $order->phoneNumber ?? '-' }}</div>
                </div>
