...
<div class="main-content">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            @foreach([''=>'All', 'pending'=>'Pending', 'confirmed'=>'Confirmed', 'in_progress'=>'In Progress', 'completed'=>'Completed', 'cancelled'=>'Cancelled'] as $val => $label)
            <a href="{{ route('front.pujariBookings', ['status'=>$val]) }}"
               style="padding:6px 14px;border-radius:20px;font-size:12px;text-decoration:none;font-weight:600;
                      {{ ($status ?? '') == $val ? 'background:#f97316;color:white;' : 'background:white;color:#64748b;border:1px solid #e2e8f0;' }}">
                {{ $label }}
            </a>
            @endforeach
        </div>
            ...
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">
                <div>
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                        <span style="font-weight:700;font-size:15px;">{{ $booking->pujaName ?? 'Session Booking' }}</span>
                        @php $badgeColors = ['pending'=>'#d97706;background:#fffbeb','confirmed'=>'#2563eb;background:#eff6ff','in_progress'=>'#0ea5e9;background:#f0f9ff','completed'=>'#16a34a;background:#f0fdf4','cancelled'=>'#dc2626;background:#fef2f2']; @endphp
                        <span style="padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;color:{{ $badgeColors[$booking->status] ?? '#64748b;background:#f1f5f9' }}">
                            {{ str_replace('_', ' ', strtoupper($booking->status)) }}
                        </span>
                    </div>
                    <p style="font-size:13px;color:#64748b;margin-bottom:4px;">
                        👤 {{ $booking->personName }} . 📞 {{ $booking->personContact }}
                    </p>
                    <p style="font-size:13px;color:#64748b;margin-bottom:4px;">
                        📅 {{ \Carbon\Carbon::parse($booking->bookingDate)->format('d M Y, l') }}
                        @if($booking->timeSlot) . 🕐 {{ $booking->timeSlot }} @endif
                    </p>
                    @if($booking->location)
                    <p style="font-size:13px;color:#64748b;">📍 {{ ucfirst($booking->location) }} @if($booking->address)— {{ $booking->address }}@endif</p>
                    @endif
                    @if($booking->specialRequirement)
                    <p style="font-size:13px;color:#94a3b8;margin-top:6px;font-style:italic;">{{ $booking->specialRequirement }}</p>
                    @endif
                </div>
                <div style="text-align:right;flex-shrink:0;">
                    <div style="font-size:20px;font-weight:700;color:#f97316;">{{ $currency }}{{ number_format($booking->totalAmount, 2) }}</div>
                    @php $pColors = ['paid'=>'#16a34a','pending'=>'#d97706','failed'=>'#dc2626']; @endphp
                    <div style="font-size:12px;color:{{ $pColors[$booking->paymentStatus] ?? '#64748b' }};font-weight:600;text-transform:capitalize;">
                        {{ $booking->paymentStatus }}
                    </div>
                    <div style="font-size:11px;color:#94a3b8;margin-top:2px;">{{ strtoupper($booking->paymentMode) }}</div>
                </div>
            </div>
