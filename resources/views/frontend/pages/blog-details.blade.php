                ...
                <div class="sticky-top" style="top: 100px;">
                    <div class="sidebar-card">
                        ...
                        @foreach ($latestBlogs as $latest)
                            ...
                            <a href="{{ route('front.getBlogDetails', $latest->slug) }}" class="sidebar-blog-item">

                                @if($isVid)
                                    <video class="sidebar-thumb-video" muted>
                                        <source src="{{ asset($latest->blogImage) }}" type="video/{{ $ext }}">
                                    </video>
                                @else
                                    <img src="{{ Str::startsWith($latest->blogImage, ['http://','https://']) ? $latest->blogImage : '/' . $latest->blogImage }}"
                                         onerror="this.onerror=null;this.src='/build/assets/images/person.png';"
                                         class="sidebar-thumb"
                                         onclick="openImage('{{ $latest->blogImage }}')"
                                         alt="{{ $latest->title }}">
                                @endif
