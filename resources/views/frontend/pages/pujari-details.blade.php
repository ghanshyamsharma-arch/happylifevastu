    ...
    <div class="row">
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body text-center pb-0">
                    <div class="mb-3">
                        <img src="{{ $pujari->profileImage ? (Str::startsWith($pujari->profileImage,'http') ? $pujari->profileImage : str_replace('storage/','public/storage/',asset($pujari->profileImage))) : '/build/assets/images/person.png' }}"
                             onerror="this.onerror=null;this.src='{{ asset('public/frontend/images/user-avatar.png') }}';"
                             alt="{{ $pujari->name }}"
                             class="rounded-circle"
                             style="width: 120px; height: 120px; object-fit: cover; border: 3px solid #FF6B35;">
                    </div>
                    <h3 class="mb-1">{{ $pujari->name }}</h3>
                    <p class="text-muted mb-2">{{ $pujari->primarySkill ?? 'Pujari' }}</p>
                    <p class="text-muted small mb-3">
                        <i class="fas fa-map-marker-alt"></i> {{ $pujari->currentCity ?? 'N/A' }}
                    </p>
                </div>
                ...
                <div class="card-body py-3 border-top border-bottom">
                    <div class="row text-center">
                        <div class="col-4 border-right">
                            <h5 class="mb-0">{{ $pujari->experienceInYears ?? 0 }}</h5>
                        ...
                        <div class="col-4">
                            <h5 class="mb-0">
                                <span class="text-warning">★</span>{{ $avgRating ?? 0 }}
                            </h5>
                ...
                <div class="card-body py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <!--    {{ $currency->value ?? '₹' }}{{ number_format($pujari->reportRate ?? 0, 0) }}-->
                        <!--</h5>-->
                    </div>
                </div>
                ...
                <div class="card-body pt-0">
                    ...
                    @if(!$isSelf)
                        <button class="btn btn-outline-secondary w-100" 
                            ...
                            <i class="fas {{ $exists ? 'fa-check' : 'fa-ban' }}"></i>
                    
                            {{ $exists ? 'Unblock' : 'Block' }}
        ...
        <div class="col-lg-8 col-md-7">
            ...
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body">
                    <p class="text-muted">{{ $pujari->description ?? 'No description available' }}</p>
                </div>
            </div>

            <!-- Services Section -->
            @if($pujas && count($pujas) > 0)
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white border-bottom">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="service-card p-3 rounded border">
                                <h6 class="mb-2">{{ $puja->name ?? 'Puja' }}</h6>
                                <p class="text-muted small mb-2">{{ $puja->categoryName ?? 'General' }}</p>
                        ...
                        <div class="review-item mb-4 pb-3 border-bottom">
                            <div class="d-flex mb-2">
                                <div class="flex-grow-1">
                                    <h6 class="mb-0" style="color:#000">{{ $rev->userName ?? 'Anonymous' }}</h6>
                            ...
                            <p class="text-muted mb-0">{{ $rev->comment ?? 'No comment' }}</p>
                        </div>
                        @endforeach
                    @else
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <div class="rating-input">
                            ...
                            <div class="stars">
                                @for($i = 1; $i <= 5; $i++)
                                <span class="star" data-value="{{ $i }}" style="cursor: pointer; font-size: 24px; color: #ddd; margin-right: 5px;">★</span>
                                @endfor
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function bookPujari() {
        @if(Auth::check())
            window.location.href = '{{ route("front.pujariBookingPage", $pujari->slug) }}';
        @else
            Swal.fire({
    ...
    function blockPujari() {
        @if(Auth::check())
            const isBlocked = {{ $exists ? 'true' : 'false' }};
...
Swal.fire({
    ...
    document.getElementById('reviewForm').addEventListener('submit', function(e) {
        ...
        @if(Auth::check())
            $.ajax({
                url: '{{ route("front.pujariSubmitReview") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    pujariId: {{ $pujari->id }},
