                                ...
                                <tbody>
                                    @forelse($orders as $order)
                                        ...
                                        @if(!empty($orderItems) && count($orderItems) > 0)
                                            ...
                                            @foreach($orderItems as $index => $item)
                                                ...
                                                <tr class="order-item-row">
                                                    {{-- Invoice Link (only on first item) --}}
                                                    <td data-label="Invoice">
                                                        @if($index === 0)
                                                            <a class="btn-pdf" 
                                                               href="{{ isset($order['invoice_link']) ? $order['invoice_link'] : '#' }}">
                                                                <i class="fa fa-file-pdf-o"></i> PDF
                                                            </a>
                                                        @endif
                                                    </td>
                                                    
                                                    {{-- Order ID (only on first item) --}}
                                                    <td data-label="Order ID">
                                                        @if($index === 0)
                                                            <strong class="amount-value">#{{ $order['id'] ?? 'N/A' }}</strong>
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td data-label="Quantity">
                                                        <span class="qty-badge">
                                                            <i class="fa fa-times mr-1"></i>{{ $item['quantity'] ?? 1 }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td data-label="Unit Price">
                                                        <span class="amount-value">
                                                            @if($walletType == 'coin')
                                                                <img src="{{ asset($coinIcon) }}" alt="Coin" width="12" style="display:inline; margin-right:2px;">
                                                            @else
                                                    ...
                                                    <td data-label="Total">
                                                        <strong class="amount-strong">
                                                            ...
                                                            @else
                                                                <span>{{ $currency['value'] ?? '' }}</span>
                                                            @endif
                                                            {{ number_format($item['totalPrice'] ?? 0, 2) }}
                                                        </strong>
                                                    </td>
                                                    
                                                    {{-- Order Date (only on first item) --}}
                                                    ...
                                                    <td data-label="Status">
                                                        @if($index === 0)
                                                            ...
                                                            @elseif($order['orderStatus'] == 'Dispatched')
                                                                <span class="status-badge status-dispatched">
                                                                    <i class="fa fa-truck mr-1"></i> Dispatched
                                                                </span>
                                                            @else
                                                                <span class="status-badge status-completed">
                                                                    <i class="fa fa-check-circle mr-1"></i> {{ $order['orderStatus'] }}
                                                                </span>
                                                            @endif
                                                        @endif
                                                    </td>
                                                    
                                                    {{-- Action (only on first item) --}}
                                                    <td data-label="Action">
                                                        @if($index === 0)
                                                            @if($order['orderStatus'] != 'Cancelled' && $order['orderStatus'] != 'Delivered' && $order['orderStatus'] != 'Dispatched')
                                                ...
                                                <td class="text-center">
                                                    <strong class="amount-strong">
                                                        ...
                                                        @else
                                                            <span>{{ $currency['value'] ?? '' }}</span>
                                                        @endif
                                                        {{ number_format($order['totalPayable'] ?? 0, 2) }}
                                                    </strong>
                                                </td>
                                                <td colspan="3"></td>
                                            </tr>
                                        @else
                                            ...
                                            <tr class="order-item-row">
                                                <td data-label="Invoice">
                                                    <a class="btn-pdf" href="{{ $order['invoice_link'] ?? '#' }}">
                                                        <i class="fa fa-file-pdf-o"></i> PDF
                                                    </a>
                                                </td>
                                                <td data-label="Order ID"><strong class="amount-value">#{{ $order['id'] ?? 'N/A' }}</strong></td>
                                                <td data-label="Product Name"><span class="product-name">{{ $order['productName'] ?? 'N/A' }}</span></td>
                                                <td data-label="Image">
                                                    @php
                                                ...
                                                <td data-label="Unit Price">
                                                    <span class="amount-value">
                                                        ...
                                                        @else
                                                            <span>{{ $currency['value'] ?? '' }}</span>
                                                        @endif
                                                        {{ number_format($order['payableAmount'] ?? 0, 2) }}
                                                    </span>
                                                </td>
                                                <td data-label="Total">
                                                    <strong class="amount-strong">
                                                ...
                                                <td data-label="Status">
                                                    ...
                                                    @else
                                                        <span class="status-badge status-completed"><i class="fa fa-check-circle mr-1"></i> {{ $order['orderStatus'] }}</span>
                                                    @endif
                                                </td>
                                                <td data-label="Action">
                                                    @if($order['orderStatus'] != 'Cancelled' && $order['orderStatus'] != 'Delivered' && $order['orderStatus'] != 'Dispatched')
